...
class Nurse  extends Entity{
    ...
    public static function findbynurse($id){
  	...
  	public static function findbynurseblock($id){
  		$sql = "SELECT *, nurse.identity as nurseId from entity inner join nurse on nurse.identity=entity.identity where entity.identity=:entity";
  		$retval =  self::$con->prepare($sql);
  		if(!$retval){
  			return null;
  		}
    ...
    public static function findAlltable($limitstart, $perpage, $order, $keyorder, $search,$idcompany){
      ...
      if(isset($idcompany)&&empty($idcompany)){
        ...
        $sqlExtra .= " AND entity.`idcompany`=".$idcompany;
  		...
  		$sql = "SELECT `entity`.`identitytype`,
                      ...
                      `entity`.`numberhelth` as numberhelth,
                       ...
                       entityUpdate.name as userUpd
                        ...
                        LEFT JOIN `nurse` ON `nurse`.identity = `entity`.identity
  		...
  		WHERE 	 entity.`statelogin`!=0 AND entity.identitytype!=1 AND entity.identitytype!=4 AND entity.identitytype!=3 AND entity.identitytype!=2 AND entity.identitytype!=6 AND   `entity`.email!='mt@sepri.pt'	AND	(UPPER( `entity`.`identity`) LIKE UPPER(:search) OR  UPPER(`entity`.numberhelth) LIKE UPPER(:search)  OR UPPER(`entity`.mobilephone) LIKE UPPER(:search)  OR UPPER(`entity`.`email`) LIKE UPPER(:search)) ".$sqlExtra." ORDER BY $keyorder $order, entity.identity DESC LIMIT :start, :limit";
  		$retval =  self::$con->prepare($sql);
  		if(!$retval){
  			return null;
  		}
    ...
    public static function countalltable($search,$idcompany){
      ...
      $sql= "SELECT count(`entity`.identity) as total 
                    ...
                    LEFT JOIN `employee` ON `employee`.identity = `entity`.identity
                    LEFT JOIN `entity` as entityUpdate ON entityUpdate.identity = `entity`.`updatedby`	
        WHERE entity.`statelogin`!=0 AND entity.identitytype!=1   AND entity.identitytype!=3 AND entity.identitytype!=4  AND entity.identitytype!=2 AND entity.identitytype!=6  ".$sqlExtra." AND `entity`.email!='mt@sepri.pt' AND (UPPER(`entity`.identity) LIKE UPPER(:search) OR  UPPER(`entity`.name) LIKE UPPER(:search) OR  UPPER(`entity`.numberhelth) LIKE UPPER(:search)  OR UPPER(`entity`.mobilephone) LIKE UPPER(:search) )";
    ...
    public static function findAlltablepdf($state){
      ...
      $sql = "SELECT `nurse`.`idnurse`,`entity`.`name` as nameentity,
                      `entity`.`photo` as photo,
                      `entity`.`email` as email,
                      `entity`.`state` as state,
                      `entity`.`mobilephone` as mobilephone,
                      ...
                      `entity`.`updatedby` as updatedbyentity ,
                      `entity`.`updatedat`as updatedatentity ,
                      entityUpdate.name as userUpd  FROM `nurse`
                                                                  INNER JOIN `entity` ON `entity`.identity = `nurse`.identity
                                                                  INNER JOIN `entity` as entityUpdate ON entityUpdate.identity = `nurse`.`updatedby`
    ...
    public static function countall1($search, $state=null,$schoolDefault){
      ...
      $sql= "SELECT count(idnurse) as total FROM `nurse`
      INNER JOIN `entity` ON `entity`.identity = `nurse`.identity
      INNER JOIN `entity` as entityUpdate ON entityUpdate.identity = `nurse`.`updatedby`
       WHERE (UPPER(idnurse) LIKE UPPER(:search)) AND entity.state=IFNULL(:state, entity.state) ".$sqlExtra."";
    ...
    public function save() {
      ...
      $sql = "INSERT INTO `nurse` (`identity`,`createdby` , `updatedby`, `createdat`, `updatedat`) VALUES
    ...
    public static function search($search='%', $limit=1, $schoolDefault){
      ...
      $sql = "SELECT *, `entity`.`identity` as id, `entity`.`name` as select_show FROM `nurse`
      INNER JOIN `entity` ON `entity`.identity = `nurse`.identity
    ...
    public static function findbynurseid($id){
      		$sql = "SELECT *,entity.name from nurse
          inner join entity on nurse.identity=entity.identity where nurse.idnurse=:idnurse";
      		$retval =  self::$con->prepare($sql);
      		if(!$retval){
      			return null;
      		}
      		$retval->bindParam(':idnurse', $id, PDO::PARAM_INT);
      		$retval->execute();
      		if ($retval->rowCount() > 0) {
      			$values = $retval->fetch(PDO::FETCH_OBJ);
      			$retval->closeCursor();
        ...
        public static function findbynursebyid($id){
              $sql = "SELECT *,entity.name,`entity`.`identitycard`,`entity`.`idschool` from nurse
              inner join entity on nurse.identity=entity.identity where nurse.identity=:identity";
              $retval =  self::$con->prepare($sql);
              if(!$retval){
                return null;
              }
        ...
        public static function searchnurse($hourstart=null, $hourend=null, $iddiscipline=null, $iddayweek=null, $search='%', $limit=1, $schoolDefault){
        ...
        /*  if(!empty($iddayweek)) {
            ...
            $joinExtra .= " INNER JOIN availability_dayweek ON availability_dayweek.idnurseavailability = nurseavailability.idnurseavailability";
          ...
          $sql = "SELECT nurse.*, nurse.identity as id, `entity`.`name` as select_show, nurseavailability.idnurseavailability FROM `nurse`
          INNER JOIN `entity` ON `entity`.identity = `nurse`.identity
          INNER JOIN nurseavailability ON nurse.identity = nurseavailability.idnurse
          ".$joinExtra."
          WHERE  entity.state=1 AND (UPPER(`entity`.`name`) LIKE UPPER(:search))  ".$sqlExtra." ".$sqlExtra1." ORDER BY nurse.identity DESC LIMIT :limit";
              $retval =  self::$con->prepare($sql);
          if(!$retval){
                  return -1;
          }
          ...
          if ($retval->rowCount() > 0) {
            ...
            while($row = $retval->fetch(PDO::FETCH_OBJ)) {
              ...
              foreach ($iddayweek as $id) {
                $sqlDay = "SELECT * FROM availability_dayweek WHERE availability_dayweek.idnurseavailability=:idnurseavailability AND availability_dayweek.iddayweek=:iddayweek";
                $retvalDay =  self::$con->prepare($sqlDay);
                $retvalDay->bindParam(':idnurseavailability', $row->idnurseavailability, PDO::PARAM_INT);
        ...
        public static function countallnurse($state=null, $schoolDefault){
         $sqlExtra1 = ""; 
          if(isset($schoolDefault)&&!empty($schoolDefault)){
            $sqlExtra1 .= " AND entity.`idschool`=".$schoolDefault;
          }
      ...
      public function delete() {
        ...
        $sql = "DELETE FROM `nurse` WHERE identity=:identity";
        $retval =  self::$con->prepare($sql);
        if(!$retval){
          return false;
        }
