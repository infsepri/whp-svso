...
if ($arr != null) {
	...
	foreach ($arr as $key => $value) {
		...
		if ($value->workcity < 5) {
			$colorworkcity = "green";
		}elseif($value->workcity >5 && $value->workcity<10){
			$colorworkcity = "warning";
		}else{
			$colorworkcity = "danger";
		}


		if ($value->workcity < 5) {
			$colorworkcity = "success";
		}elseif($value->workcity >5 && $value->workcity<10){
			$colorworkcity = "warning";
		}else{
			$colorworkcity = "danger";
		...
		if ($value->workload < 5) {
			$colorworkload = "success";
		}elseif($value->workload >5 && $value->workload<10){
			$colorworkload = "warning";
		}else{
			$colorworkload = "danger";
		}

		if ($value->workenvironment < 5) {
			$colorworkenvironment = "success";
		}elseif($value->workenvironment >5 && $value->workenvironment<10){
			$colorworkenvironment= "wuarnig";
		}else{
			$colorworkenvironment = "danger";
		}
...
?>
		...
		<tr>
			...
			<td class="col-xs-1 col-sm-1 col-md-1 col-lg-1" onclick="golocationspecifyurl('?controller=admin&action=edit&id=<?php echo $id_hash; ?>&t=<?php echo $id_hash1; ?>');"><?php echo ((isset($value->mobilephone) && !empty($value->mobilephone)) ? $value->mobilephone : "N/D"); ?>

		

			</td>
			<td class="col-xs-1 col-sm-1 col-md-1 col-lg-1" onclick="golocationspecifyurl('?controller=notify&action=getall&id=<?php echo $id_hash; ?>&t=<?php echo $id_hash1; ?>');">



				<i class='clip-health'></i><span class="badge badge-<?php echo $colorworkload ?>"> <?php echo $value->workload; ?></span>

			</td>


			<td class="col-xs-1 col-sm-1 col-md-1 col-lg-1" onclick="golocationspecifyurl('?controller=notify&action=getall&id=<?php echo $id_hash; ?>&t=<?php echo $id_hash1; ?>');">
				<i class='clip-target'></i><span class="badge badge-<?php echo $colorworkcity?>"><?php echo $value->workcity; ?></span>
			</td>
			<td class="col-xs-1 col-sm-1 col-md-1 col-lg-1" onclick="golocationspecifyurl('?controller=notify&action=getall&id=<?php echo $id_hash; ?>&t=<?php echo $id_hash1; ?>');">
				<i class='clip-brightness-high'></i><span class="badge badge-<?php echo $colorworkenvironment?>"><?php echo $value->workenvironment; ?></span>
			</td>

			<td class="col-xs-1 col-sm-1 col-md-1 col-lg-1" onclick="golocationspecifyurl('?controller=notify&action=getall&id=<?php echo $id_hash; ?>&t=<?php echo $id_hash1; ?>');"><?php echo $value->userUpd; ?></td>
			<td class="col-xs-1 col-sm-1 col-md-1 col-lg-1" onclick="golocationspecifyurl('?controller=notify&action=getall&id=<?php echo $id_hash; ?>&t=<?php echo $id_hash1; ?>');"><?php echo $value->updatedatentity; ?></td>
			...
			<?php } else {  ?>
				<td style="width:10%;" onclick="confirm_Swal('1', ['<?php echo $title; ?>', '<?php echo $title_desc; ?>', '?controller=admin&action=changestate&id=<?php echo $value->identity; ?>'])">
					<?php if ($value->state == 1) { ?>
						<span class="cPointer label label-green hidden-sm hidden-xs" id="badgesSameSize"><?php echo $lang['active_state']; ?></span>
						<span class='cPointer hidden-lg hidden-md circle_ative' title="<?php echo $lang['active_state']; ?>"><i class='fa fa-circle circle'></i></span>
	...
	<tr></tr>
	<tr>
		<td colspan="7"><?php echo $lang['table_withoutdata']; ?></td>
	</tr>
<?php
}
?>