...
function checkreference(){
  ...
  if(inputVal!=""){
    $.ajax({
      url: '?controller=reference&action=checkreference',
      type: 'POST',
      data: { 'reference[code]': inputVal} ,
      success: function (response) {
        response=JSON.parse(response);
        ...
        if(response.code==200){
          ...
          $('#referenceexist').show();
          $('#codesubmit').hide();
          $("#code").css("border", "3px solid #FF0000");
          $('#referenceexist').attr("disabled", true);
        }else{
         
          $('#referenceexist').hide();
          $('#codesubmit').show();
          $("#code").css("border", "3px solid #9ecaed");
